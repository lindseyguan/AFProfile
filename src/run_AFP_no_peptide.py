...
def predict_structure(
  ...
  for model_index, (model_name, model_runner) in enumerate(model_runners.items()):
    ...
    with open(fasta_path, 'r') as f:
      lines = f.readlines()
      protein_length = len(lines[1].strip())
      peptide_length = len(lines[3].strip())

    print('Protein length:', protein_length)
    print('Peptide length:', peptide_length)
    print('Freezing peptide MSA cluster bias...')

    # Mask peptide MSA from processed feature dict
    # print(processed_feature_dict['msa'])
    for i in range(1, msa_shape[0]): # rows
      for j in range(msa_shape[1] - peptide_length, msa_shape[1]): # columns
        processed_feature_dict['msa'][i][j] = 21 # make gaps
    # print(processed_feature_dict['msa'])

    #Need to init zeros = no influence on the cluster profile
    msa_params_opt = jnp.zeros((config.model.embeddings_and_evoformer.num_msa, msa_shape[1] - peptide_length, 23), dtype='float32') #23 classes for aa, gap, mask
    peptide_zeros = jnp.zeros((config.model.embeddings_and_evoformer.num_msa, peptide_length, 23), dtype='float32')
    msa_param_af = jnp.concat([msa_params_opt, peptide_zeros], axis=1)

    print('Reconstituted params shape:', msa_param_af.shape)
    optimizer = optax.adam(learning_rate)
    opt_state = optimizer.init(msa_params_opt)

    def loss_fn(msa_params_opt, processed_feature_dict):
        """Calculate loss
        ...
        """
        msa_param_af = jnp.concat([msa_params_opt, peptide_zeros], axis=1)
        prediction_result = model_runner.predict(msa_param_af, processed_feature_dict) #The params for model runner are contained within self
        #Get confidence
    ...
    def update(msa_params_opt, opt_state, processed_feature_dict):
        (loss, aux), grads = jax.value_and_grad(loss_fn, has_aux=True)(msa_params_opt, processed_feature_dict)

        updates, new_opt_state = optimizer.update(grads, opt_state)
        new_params = optax.apply_updates(msa_params_opt, updates)
    ...
    for i in range(max_iter):
        ...
        #Predict
        t0= time.time()
        loss, aux, msa_params_opt, opt_state = update(msa_params_opt, opt_state, processed_feature_dict)
        t1 = time.time()
        ...
        with open(bias_path, 'wb') as f:
            np.save(f, np.array(msa_params_opt))
